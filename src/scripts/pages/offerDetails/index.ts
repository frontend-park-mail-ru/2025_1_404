...
export default class OfferDetailsPage extends Page {
    ...
    render({layout, root, props}: PageRenderInterface) {
        ...
        .then ((data) => {
            ...
            offerDetailsHeader.innerHTML = offerDetailsHeaderTemplate({verified: offer.verified, propertyType: offer.propertyType.toLowerCase(), inMultipleForm: offer.propertyType.toLowerCase() === 'апартаменты', isRent: offer.offerType === 'Аренда',rooms: offer.rooms, area: offer.area, price: offer.price, floor: offer.floor, totalFloors: offer.totalFloors, metroStation: offer.metroStation || 'Нет', metroColor: offer.metroColor || '999999', address: offer.address});
